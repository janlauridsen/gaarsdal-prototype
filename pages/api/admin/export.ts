...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const allConversations: Array<{ conversation_id: string; user_key?: string; turns: RawTurn[] }> = []
    ...
    for (const convId of conversationIds) {
      ...
      // Hent user_key fra første event — bruges til gruppering i admin
      let userKey: string | undefined
      try {
        const firstEvent = await redis.lrange(`gaarsdal:events:v1:conv:${convId}`, 0, 0) as unknown[]
        if (firstEvent?.length) {
          const ev = typeof firstEvent[0] === "string" ? JSON.parse(firstEvent[0] as string) : firstEvent[0]
          if (typeof (ev as any)?.user_key === "string") userKey = (ev as any).user_key
        }
      } catch { /* non-fatal */ }

      allConversations.push({ conversation_id: convId, user_key: userKey, turns })
